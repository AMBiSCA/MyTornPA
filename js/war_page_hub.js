document.addEventListener('DOMContentLoaded', function() {
    ...
    function processFactionDataFromTornApi(id, resp) {
        const d = {
            factionName: "Unknown",
            members: [],
            totalMembers: 0 // Initialize totalMembers for safety
        };
        if (resp.error) throw new Error(`API Error for ${id}: ${resp.error.error}`);
        d.factionName = resp.name || `Faction ${id}`;

        // Check if resp.members is an object (from selections=members or basic)
        if (resp.members && typeof resp.members === 'object') {
            // If it has a 'total' property (from basic selection)
            if (typeof resp.members.total === 'number') {
                d.totalMembers = resp.members.total;
            } else { // If it's a list of individual members (from members selection or basic without total)
                ...
                d.totalMembers = d.members.length; // Count if total property is missing
            }
        }
        return d;
    }
    ...
    async function fetchAndProcessFactionActivity() {
        ...
        try {
            ...
            let enFid = enemyFactionIDInput.value.trim(); // Use 'let' as it might be updated

            if (myFid === '') { // My Faction ID is mandatory
                updateStatus("Error.", 'error', "Please enter your Faction ID before starting!");
                stopButton.click();
                return;
            }

            // --- First API Call: Fetch My Faction Data (including ranked_wars to find enemy ID) ---
            const myFactionUrl = `https://api.torn.com/faction/${myFid}?selections=basic,ranked_wars,members&key=${tornApiKey}`; // Added 'ranked_wars' and 'members'
            const myResp = await fetchTornApi(myFactionUrl);
            const myData = processFactionDataFromTornApi(myFid, myResp);

            // --- Attempt to auto-detect enemy faction ID if not manually provided ---
            if (enFid === '' && myResp.ranked_wars?.current?.opponent_faction_id) {
                enFid = myResp.ranked_wars.current.opponent_faction_id;
                console.log(`Auto-detected enemy faction ID from ranked war: ${enFid}`);
                // Optionally, update the enemyFactionIDInput field visually for the user
                // enemyFactionIDInput.value = enFid;
            }

            // --- Validate Enemy Faction ID before proceeding ---
            if (enFid === '') {
                updateStatus("Error.", 'error', "Enemy Faction ID not provided or no active ranked war found!");
                stopButton.click();
                return;
            }

            // --- Second API Call: Fetch Enemy Faction Data ---
            const enemyFactionUrl = `https://api.torn.com/faction/${enFid}?selections=basic,members&key=${tornApiKey}`; // Using 'basic,members' for enemy
            const enResp = await fetchTornApi(enemyFactionUrl);
            ...
            totalMembersMyFactionDisplay.textContent = `${myData.totalMembers} | ${enData.totalMembers}`; // Use totalMembers from processed data
            const rec = {
                ...
                myFaction: {
                    ...
                    totalMembers: myData.totalMembers,
                ...
                enemyFaction: {
                    ...
                    totalMembers: enData.totalMembers,
    ...
    function enterRunningState(sessionState) {
        ...
        // IMPORTANT: The Faction IDs are fetched from inputs at the start of fetchAndProcessFactionActivity
        // so no need to explicitly set them here from sessionState.
        // myFactionIDInput.value = sessionState.myFactionID;
        // enemyFactionIDInput.value = sessionState.enemyFactionID;

        fetchAndProcessFactionActivity(); // This will now use the current input values
    ...
    async function generateAndDownloadFactionWideReport() {
        if (historicalData.length < 2) {
            alert("Not enough data has been recorded for a full report. Please start tracking to generate a report.");
            return;
        }
    ...
    if (typeof firebase !== 'undefined' && typeof firebase.auth !== 'undefined' && typeof firebase.firestore !== 'undefined') { // Firebase initialized check
    ...
    startButton.addEventListener('click', () => {
        ...
        if (myFactionIdValue === '') { // My Faction ID is now the minimum mandatory input
            updateStatus("Please enter your Faction ID before starting!", 'info');
            return;
        }
        ...
        const sessionState = {
            ...
            enemyFactionID: enemyFactionIdValue, // Store potentially empty enemy ID for session state
