/* ==========================================================================
   War Page Hub JavaScript (war_page_hub.js) - v12 (FINAL w/ Bug Fix)
   ========================================================================== */
...
const designateAdminsContainer = document.getElementById('designatedAdminsContainer');
...
async function initializeAndLoadData(apiKey) {
    try {
        ...
        // *** FIX ***: Pass the newly fetched faction data directly into the listener function.
        // This prevents a race condition where the listener might run before the global variable is ready.
        listenForLiveMemberData(factionApiFullData);
...
// *** FIX ***: Function now accepts the faction data as a parameter to avoid relying on a global variable.
function listenForLiveMemberData(fullFactionData) {
    if (!fullFactionData || !fullFactionData.members) {
        console.error("Cannot listen for live data without the main faction member list.");
        return;
    }
    ...
    liveDataRef.onSnapshot(snapshot => {
        ...
        const combinedData = liveMembers.map(liveMember => {
            const staticData = fullFactionData.members[liveMember.id] || {};
            return { ...staticData, ...liveMember }; 
        });
...
function renderFriendlyStatusTable(memberData) {
    ...
    for (const member of memberData) {
        ...
        const hospitalTimer = member.status ? (member.status.until - Math.floor(Date.now() / 1000) > 0 ? member.status.until : 0) : 0;
        const travelingTimer = member.status ? (member.status.until - Math.floor(Date.now() / 1000) > 0 ? member.status.until : 0) : 0;
        ...
        tableHtml += `
            <tr>
                ...
                <td>${(member.status?.description || '').includes('Traveling') ? 'Yes' : 'No'}</td>
                <td>${(member.status?.description || '').includes('Hospital') ? 'Yes' : 'No'}</td>
            </tr>
        `;
    }
...
function setupEventListeners(apiKey) {
    ...
    if (saveSelectionsBtnBH) {
        saveSelectionsBtnBH.addEventListener('click', async () => {
            if (!bigHitterWatchlistContainer) return;
            const selectedWatchlistIds = Array.from(bigHitterWatchlistContainer.querySelectorAll('input:checked')).map(cb => cb.value);
            try {
