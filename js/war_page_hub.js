...
async function sendAvailabilityReport() {
    ...
    try {
        ...
        // 5. Format the report into separate message chunks
        const messageChunks = [];
        
        // Chunk 1: Header
        messageChunks.push(`**War Availability Report for ${factionApiFullData.basic.name}**\n-----------------------------------`);

        // Chunks 2, 3, 4: Each day's summary
        for (let i = 1; i <= 3; i++) {
            const dayKey = `day${i}`;
            const dayReport = report[dayKey];
            let dayString = `\n**DAY ${i} SUMMARY**\n`;
            dayString += `✅ **Available (${dayReport.yes.length}):** ${dayReport.yes.join(', ') || 'None'}\n`;
            dayString += `🟧 **Partially (${dayReport.partial.length}):** ${dayReport.partial.join(', ') || 'None'}\n`;
            dayString += `❌ **Unavailable (${dayReport.no.length}):** ${dayReport.no.join(', ') || 'None'}\n`;
            dayString += `❔ **No Response (${dayReport.noResponse.length}):** ${dayReport.noResponse.join(', ') || 'None'}`;
            messageChunks.push(dayString);
        }
        
        // Chunk 5: Footer
        messageChunks.push("-----------------------------------\nReport generated by MyTornPA.");
        
        // 6. Send each chunk to the backend one by one
        const backendWebhookEndpoint = '/.netlify/functions/send-availability-webhook';
        ...
        for (let i = 0; i < messageChunks.length; i++) {
            ...
            if(button) button.textContent = `Sending... (${i+1}/${messageChunks.length})`;
            ...
            const response = await fetch(backendWebhookEndpoint, {
                ...
                body: JSON.stringify({
                    discordWebhookUrl: discordWebhookUrl,
                    reminderMessage: chunk, // Send the chunk in the field the backend expects
                    nonResponders: [],      // Send an empty list
                    factionName: factionApiFullData.basic.name,
                    factionId: globalYourFactionID
                })
            });
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(`Backend Function failed on chunk ${i+1}: ${errorData.message || response.statusText}`);
            }

            // Wait 1 second before sending the next chunk to avoid rate-limiting
            if (i < messageChunks.length - 1) {
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
        }

        console.log("Availability report successfully dispatched in chunks.");
