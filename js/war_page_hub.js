...
async function updateFriendlyMembersTable(apiKey, firebaseAuthUid) {
    ...
    tbody.innerHTML = '<tr><td colspan="12" style="text-align:center; padding: 20px;">Loading faction member stats from database...</td></tr>'; // Increased colspan
    ...
    try {
        ...
        // We now directly rely on the faction_id being in the userProfile
        ...
        // --- NEW LOGIC: Fetch members directly from the 'users' collection in Firestore ---
        // Query for all documents in the 'users' collection where 'faction_id' matches the current user's faction_id.
        ...
        if (factionMembersSnapshot.empty) {
            tbody.innerHTML = '<tr><td colspan="12" style="text-align:center; padding: 20px;">No members found in your faction in the database, or data not yet updated.</td></tr>';
            return;
        }
        ...
        for (const member of membersArray) {
            ...
            // This is crucial because document IDs are now "Name [ID]" strings
            ...
            allRowsHtml += `
                <tr data-id="${tornPlayerId}" class="${statusClass}"> <td><a href="${profileUrl}" target="_blank">${name}</a></td>
                    <td>${position}</td>
                    <td>${level}</td>
                    <td>${lastAction}</td>
                    <td>${strength}</td>
