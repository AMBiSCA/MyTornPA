document.addEventListener('DOMContentLoaded', function() {
    ...
    const clearDataButton = document.getElementById('clearDataButton'); // This is the 'Clear/Download Data' button on the main page
    ...
    const compareUser2Select = document.getElementById('compareUser2'); // CORRECTED: Now targets "compareUser2" from HTML
    ...
    const confirmClearBtn = document.getElementById('confirmClearBtn'); // This is the 'Clear Data' button inside the "Session Complete" modal
    ...
    const compareTwoIndividualsBtn = document.getElementById('compareTwoIndividualsBtn'); // This is the 'Compare Two Individuals' button in the report options modal
        ...
        if (resp.members) {
            Object.entries(resp.members).forEach(([mId, m]) => {
                ...
                if (memberIdString !== 'INVALID_ID' && m.name) {
                    d.members.push({ id: memberIdString, name: m.name, minutesAgo: mins, active: mins <= 15 ? 1 : 0, display: `${m.name} [${memberIdString}]` });
                } else {
    ...
    function updateCustomReportChart() { // Removed 'isMyFaction' parameter as it's no longer needed for this logic
        // Always set both main comparison selects based on the report modal's current selections
        // This ensures both can be selected simultaneously for comparison
        compareUser1Select.value = reportMyFactionMemberSelect.value;
        compareUser2Select.value = reportEnemyFactionMemberSelect.value;
    ...
    async function generateAndDownloadIndividualComparisonReport() {
        ...
        const isMyMemberIdValid = selectedMyMemberId && selectedMyMemberId !== "" && selectedMyMemberId !== "undefined" && selectedMyMemberId !== "null";
        const isEnemyMemberIdValid = selectedEnemyMemberId && selectedEnemyMemberId !== "" && selectedEnemyMemberId !== "undefined" && selectedEnemyMemberId !== "null";

        if (!isMyMemberIdValid && !isEnemyMemberIdValid) {
        ...
        const selectedMyMember = isMyMemberIdValid ? ( // Only try to find if ID is valid
            historicalData.map(r => r.myFaction.individuals.find(m => String(m.id) === String(selectedMyMemberId))).filter(Boolean)[0] || null
        ) : null;
        const selectedEnemyMember = isEnemyMemberIdValid ? ( // Only try to find if ID is valid
            historicalData.map(r => r.enemyFaction.individuals.find(m => String(m.id) === String(selectedEnemyMemberId))).filter(Boolean)[0] || null
        ) : null;
        
        // --- CORRECTED: More robust error handling for when selected member is NOT found ---
        if (isMyMemberIdValid && !selectedMyMember) {
            alert(`My Faction Member (ID: ${selectedMyMemberId}) not found in historical data. This usually means corrupted historical data or an outdated selection.`);
            return;
        }
        if (isEnemyMemberIdValid && !selectedEnemyMember) {
            alert(`Enemy Faction Member (ID: ${selectedEnemyMemberId}) not found in historical data. This usually means corrupted historical data or an outdated selection.`);
            return;
        }
        // --- END CORRECTED ---
    ...
    // Corrected: clearDataButton (main page 'Clear/Download Data') should open the report modal
    ...
    // Corrected: Ensure this listener is directly on DOMContentLoaded, not nested
...
    reportEnemyFactionMemberSelect.addEventListener('change', () => updateCustomReportChart());
    ...
    // This is the correct clear data button listener from the modal
