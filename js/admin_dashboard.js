...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.admin-tool-container').appendChild(dataDisplayArea);
    ...
    function showMainError(message) {
        ...
        const containerDiv = document.querySelector('.admin-tool-container');
        if (containerDiv) {
            const inputGroup = containerDiv.querySelector('.input-group');
            if (inputGroup) {
    ...
    // This function remains in client-side JS as it's for display formatting
    ...
    fetchFactionDataBtn.addEventListener('click', async () => {
        ...
        try {
            // URL of your Netlify Function (replace 'getFactionData' with your function's name)
            // Make sure your Netlify Function is deployed and accessible.
            const netlifyFunctionUrl = '/.netlify/functions/getFactionData'; // Common Netlify Functions path

            const response = await fetch(netlifyFunctionUrl, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    // Optional: You could send the user's Firebase ID token here for authentication
                    // if your Netlify Function needs to verify the admin status itself.
            ...
            const result = await response.json(); // The data returned by your Netlify Function
            ...
            if (result.members && result.members.length > 0) {
                combinedFactionMembersData = result.members; // Store globally for saving
                ...
                renderDataTable(combinedFactionMembersData); // Display the data in the table
                saveToFirebaseBtn.style.display = 'block'; // Show the save button
            } else {
            ...
            dataDisplayArea.innerHTML = ''; // Clear display on error
        }
    });
    ...
    saveToFirebaseBtn.addEventListener('click', async () => {
        ...
        try {
            const batch = db.batch(); // Use a Firestore batch for efficiency
            let savedCount = 0;

            combinedFactionMembersData.forEach(memberRecord => {
                const memberId = String(memberRecord.memberId); // Ensure member ID is string for document ID
                const docRef = db.collection("playerdatabase").doc(memberId);
                // Ensure the record is structured correctly for Firestore
                const recordToSave = { ...memberRecord };
                // Firebase serverTimestamp() can only be set at the top level or within a map,
                // not inside nested objects if they are just raw data from an external API.
                // It's already added per member record in the Netlify Function's expected output.

                batch.set(recordToSave, { merge: true }); // Use merge to update existing or create new
                savedCount++;
            });
            ...
            saveToFirebaseBtn.style.display = 'none'; // Hide button after saving
            factionIdInput.value = ''; // Clear input field
            dataDisplayArea.innerHTML = ''; // Clear display table
        } catch (error) {
            ...
            saveToFirebaseBtn.disabled = false; // Re-enable button
        }
    });
});