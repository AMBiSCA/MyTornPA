...
async function switchFactionOverviewSubTab(tabId) {
    ...
    switch (tabId) {
        ...
        case 'logistics':
            ...
            // REVERTED to show original headers, to be removed later with clarification
    ...
    if (data.length === 0) {
        ...
        data.forEach(row => {
            ...
            columns.forEach(col => {
                ...
                // If displayValue is still N/A, 'o', or '0', display as empty string
                if (displayValue === 'N/A' || displayValue === 'o' || displayValue === 0) { // Added 0 as a check as user reported it
        ...
        header.addEventListener('click', () => {
            ...
            if (tbody) {
                ...
                sortedData.forEach(row => {
                    ...
                    columns.forEach(col => { // Use 'columns' from switchFactionOverviewSubTab's scope
                        const colKey = col.key; // Use col.key directly
                        ...
                        if (colKey === 'timestamp' && displayValue !== 'N/A') {
                            ...
                            displayValue = `$${displayValue.toLocaleString()}`;
                        } else if (colKey === 'quantity' && typeof displayValue === 'number') {
                            displayValue = displayValue.toLocaleString();
                        }
                         // If displayValue is N/A, 'o', or '0', display as empty string
                        if (displayValue === 'N/A' || displayValue === 'o' || displayValue === 0) {
                            displayValue = ''; // Display as empty string in table for N/A or default 0
                        }
