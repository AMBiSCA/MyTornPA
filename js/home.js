...
    if (authModal) {
        authModal.classList.add('is-active'); // Assuming is-active makes it visible
...
    personalStatsModal.classList.add('visible'); // Corrected: Use classList.add('visible')
...
            const statusState = statusObject.state || "";
            const statusDesc = statusObject.description || "";
            console.log(`Hospital Check (from data.status) - Values: until: <span class="math-inline">\{hospitalUntil\}, state\: '</span>{statusState}', desc: '${statusDesc}'`);
...
        personalStatsModal.classList.remove('visible'); // Corrected: Use classList.remove('visible')
        if (togglePersonalStatsCheckbox) togglePersonalStatsCheckbox.checked = false;
    });
  } else {
...
  function showProfileSetupModal() { if (profileSetupModal) profileSetupModal.classList.add('visible'); } // Corrected: Use classList.add('visible')
  function hideProfileSetupModal() { if (profileSetupModal) { profileSetupModal.classList.remove('visible'); if (nameErrorEl) nameErrorEl.textContent = ''; if (profileSetupErrorEl) profileSetupErrorEl.textContent = ''; } } // Corrected: Use classList.remove('visible')
...
                    // --- CODE ALREADY ADDED HERE FOR LOGIN FACTION UPDATE ---
...
});                                           do not do one absoloyute thing at all yet. need to see the info yuouve taken in from the js ive sent to help us with the page,