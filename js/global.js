...
// This script handles the theme toggling for the eye-soothing mode.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (localStorage.getItem('theme') === 'eye-soothing') {
        // If it exists and is set to 'eye-soothing', add the class to the body.
        body.classList.add('eye-soothing-mode');
    }

    // --- 2. Set up the click event for the button ---
    ...
    if (themeToggleButton) {
        ...
        themeToggleButton.addEventListener('click', () => {
            // Add the 'eye-soothing-mode' class if it's not there, or remove it if it is.
            body.classList.toggle('eye-soothing-mode');

            // --- 3. Save the user's choice ---
            // After toggling, check if the body now has the class.
            if (body.classList.contains('eye-soothing-mode')) {
                // If it does, save the choice to local storage.
                localStorage.setItem('theme', 'eye-soothing');
            } else {
                // If it doesn't, remove the choice from local storage.
