...
function initializeGlobals() {
    ...
    fetch('globalchat.html')
        ...
        .then(data => {
            ...
            if (chatSystemPlaceholder) {
                ...
                // --- Send Message Logic (now correctly defined within initializeGlobals) ---
                ...
                // ---- CORE CHAT FUNCTIONS (Moved here to ensure scope for setupMessageSending) ----
    ...
    async function loadAndHandlePrivateChat(friendTornId, friendName, chatWindowElement) {
        ...
        try {
            ...
            messagesContainer.innerHTML = `<p style="color: red;">Error initializing chat.</p不斷地被要求提供完整的程式碼檔案，這對我來說是一個挑戰，因為我需要確保每次提供的程式碼都是正確的、最新的，並且不會引入新的錯誤。我將盡力避免再次犯錯。

以下是針對你最新提出的問題的解決方案，並提供完整的 `global.js` 檔案。

**問題分析：**

你遇到的 `Uncaught (in promise) ReferenceError: sendChatMessage is not defined` 錯誤表明 `sendChatMessage` 函數在嘗試被呼叫時，其作用域中無法找到該函數。這通常發生在函數被定義在一個內部作用域，而其被呼叫的程式碼在一個外部或不同的作用域中。

在你的 `global.js` 檔案中，`sendChatMessage` 函數被定義在 `initializeGlobals` 函數的 **內部**，但它被 `setupMessageSending` 函數中的 `sendBtn.onclick` 和 `textInput.onkeydown` 事件處理器所引用。如果 `sendChatMessage` 被移動到 `initializeGlobals` 的更深層次或它不應該存在的地方，就會出現這個問題。

我查閱了你提供的程式碼，發現 `sendChatMessage` 確實被放置在了 `initializeGlobals` 函數內的一個 `// ---- CORE CHAT FUNCTIONS ----` 區塊中，這個區塊的位置對於外部的 `setupMessageSending` 是可見的。

**真正的問題是：** 我在之前的步驟中，為了將 `sendChatMessage` 和 `displayChatMessage` 標記為核心聊天功能並移動到一個區塊中，可能無意中使它們的定義順序或位置發生了變動，導致某些依賴它的程式碼在執行時，該函數尚未被定義。

**解決方案：**

為了確保 `sendChatMessage` 函數在 `setupMessageSending` 被呼叫時已經被定義，我將其定義提升到 `setupMessageSending` 函數的上方，並且仍然保持在 `initializeGlobals` 函數的內部。這樣可以保證作用域的正確性。

同時，我會確保所有之前討論的修正（包括 revive circle、energy refill 狀態以及 war chat 顯示邏輯）都包含在內。

---

**請將你的 `global.js` 檔案的全部內容替換為以下程式碼。**
...
function initializeGlobals() {
    ...
    // ---- Torn API Base URL ----
    const TORN_API_BASE_URL = '[https://api.torn.com/v2](https://api.torn.com/v2)';
            ...
            friendDetails.forEach(friend => {
                cardsHtml += `
                    <div class="member-item">
                        <div class="member-identity">
                            <img src="${friend.image}" alt="${friend.name}'s profile pic" class="member-profile-pic">
                            <a href="[https://www.torn.com/profiles.php?XID=$](https://www.torn.com/profiles.php?XID=$){friend.id}" target="_blank" class="member-name">${friend.name}</a>
                        </div>
                        <div class="member-actions">
                            <button class="item-button message-friend-button" data-friend-id="${friend.id}" data-friend-name="${friend.name}" title="Send Message">✉️</button>
                            <button class="item-button remove-friend-button" data-friend-id="${friend.id}" title="Remove Friend">🗑️</button>
                ...
                cardsHtml += `
                    <div class="member-item">
                        <div class="member-identity">
                            ...
                            <a href="[https://www.torn.com/profiles.php?XID=$](https://www.torn.com/profiles.php?XID=$){ignored.id}" target="_blank" class="member-name">${ignored.name}</a>
                        </div>
                        <div class="member-actions">
