...
exports.handler = async function(event, context) {
    ...
    try {
        ...
        // 2. Select 6 random targets from the list
        const shuffledTargets = shuffleArray(potentialTargets);
        const selectedTargets = shuffledTargets.slice(0, 6);
        console.log(`Selected ${selectedTargets.length} random targets to check.`);

        if (selectedTargets.length === 0) {
            return { statusCode: 200, body: JSON.stringify({ message: "No suitable targets found to recommend." }) };
        }


        // 3. Get real-time status for the selected targets
        ...
        // 4. Combine data and format the final response
