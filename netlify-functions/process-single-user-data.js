...
exports.handler = async (event, context) => {
    ...
    try {
        // We only need the 'personalstats' selection for refills. The rest of the data is also fetched.
        ...
        // Get stored counts and statuses from our database
        ...
        // Check if it's a new user (no last update) or a new day
        if (!lastUpdateTimestamp || lastUpdateTimestamp.getUTCFullYear() !== now.getUTCFullYear() || lastUpdateTimestamp.getUTCMonth() !== now.getUTCMonth() || lastUpdateTimestamp.getUTCDate() !== now.getUTCDate()) {
            // It's a new day or the first time we're checking. Reset the daily status.
            energyRefillUsed = false;
            nerveRefillUsed = false;
        }

        // Now, check if the counter has increased since the last check
        ...
        const userDataToSave = {
            ...
            nerve// Import Firebase Admin SDK and node-fetch
const admin = require('firebase-admin');
const fetch = require('node-fetch');
