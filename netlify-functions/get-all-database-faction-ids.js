...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function processNextBatch() {
        ...
        try {
            ...
            updateStatus(`Error during processing. Check console for details.`, true); // Generic error message for display
        ...
        refreshDatabaseFactionsBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (result.success && result.factionIds && result.factionIds.length > 0) {
                    ...
                    updateStatus(`Status: Running. Found ${allDbFactionsToProcess.length} unique factions in your database. Starting refresh process...`, false); // Initial more detailed message for this specific step
    ...
    async function processNextDbFaction() {
        ...
        // Only update a generic "Running" message. No per-faction message here.
        updateStatus(`Status: Running.`, false); 
        ...
        try {
            ...
            if (result.success && result.memberIDs && result.memberIDs.length > 0) {
                ...
                updateStatus(`Status: Running. No members found for database Faction ID ${currentFactionId}. Skipping.`, true); // Brief internal info, but still running overall
